...
|     命令      |        说明        |
|:-------------:|:--------------------------:|
| api列表 / API列表 | 查看已收录 API 分类与数量 |
| api详情 / API详情 xxx  | 具体查看某个api的参数，不提供参数时查看所有能触发api的关键词 |
| api测试 / API测试 | 轮询测试所有 API 可用性 |
| 添加api / 添加API | 管理员添加 API（支持 CLI 参数格式与旧详情粘贴格式） |
| 删除api / 删除API xxx  | 删除指定api        |
|   {关键词}     |   触发api      |

建议直接通过编辑"data\plugins\astrbot_plugin_apis\api_data.json"进行添加api、删除api，同时也方便修改更多参数。

- "keyword": api触发词，支持设置多个。
- "fuzzy": 当为 true 时表示“包含匹配”，当为 false 时表示“精准匹配”。
...
- `-v` / `--video`：类型设为 `video`
- `-a` / `--audio`：类型设为 `audio`
- `-u` / `--url <URL>`：请求地址（必填，可重复填写多个 URL，失败自动轮询）
- `-k` / `--keyword <词1,词2>`：触发词（可选，逗号分隔；不填时默认使用 API 名）
- `-p` / `--param <key=value>` 或 `-p <key>`：请求参数（可重复）
- `-g` / `--target <路径>`：JSON 解析路径（如 `data.msg`）
- `-f` / `--fuzzy <true|false>`：是否包含匹配（可选）

示例：

```plaintext
添加API 笑话 -t -u https://free.wqwlkj.cn/wqwlapi/xiaohua.php -p type=text -f false
```

```plaintext
添加API 艺术字 -i -u https://free.wqwlkj.cn/wqwlapi/ysz.php -p text=你好 -p id=30 -g data.imgurl
```

```plaintext
添加API 天气 -t -u https://api1.example.com/weather -u https://api2.example.com/weather -p city=北京 -g data.msg
```
...
- 枫林API二代: [https://api-v2.yuafeng.cn](https://api-v2.yuafeng.cn)
...
- 龙珠API: [https://sdkapi.hhlqilongzhu.cn](https://sdkapi.hhlqilongzhu.cn)
